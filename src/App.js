...
function Page({ text, secondtext, thirdtext, images, textScaleFactor, onReflow, left = false }) {
  ...
  return (
    <Box dir="column" align={left ? 'flex-start' : 'flex-end'} justify="flex-start" width="100%" height="auto" minHeight="100%">
      ...
      <Box marginLeft={left ? 1.5 : 1} marginTop={1} marginRight={left ? 1 : 1.5} marginBottom={1}>
        <Text
          bold
          position-z={0.5}
...
function Content({ onReflow }) {
  const group = useRef()
  const { viewport, size } = useThree()
  // zoom animation width and height
